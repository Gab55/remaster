...
public class Jeu {
    ...
    private ArrayList<Joueur> listEquipe;
    ...
    public void debuter() {
        this.listEquipe= new ArrayList<Joueur>();
    ...
    // création d'une équipe d'amoureux
    public void cupidon(Joueur j){
        int tailleMax = this.nbJoueurs;
        int tailleMin = 0;
        Random ra1 = new Random();
        int perso1 = tailleMin + ra1.nextInt(tailleMax-tailleMin);
        Joueur joueurAmoueur =new Joueur(j.getNom(),j.getCarte());
        for (int i =0; i<tabJoueur.size();i++) {
            if ((listEquipe.size() != 2) && (perso1==tabJoueur.get(i).getCarte())) {
                this.listEquipe.add(joueurAmoueur);
                perso1=0;
            }
        }
        System.out.println("la team d'amoueux est bien là! " + listEquipe.size());
    ...
    public void choixVillageois() {
        ...
        for (int i = 0; i < tabJoueur.size(); i++) {
            while (!(b)) {
                // boucle pour printer que c'est au tour d'un personne de l'équipe d'amoureux
                for (int z = 0; z < listEquipe.size(); z++){
                    if (tabJoueur.get(i).getNom().equals(listEquipe.get(z).getNom())){
                        System.out.println("A vous les putain d'amoureux");
                    }
                }
    ...
    public void role(Joueur j) { //Ajoute ou non(aléatoire) un joueur à la liste des loups garous s'il ils ne sont pas 2
        ...
        if ((fonction==2) && (listLp.size()!=2)){
            ...
            listLp.add(lp);
            //code peut etre inutile car on donne des indications aux joueurs
            //
            //
            //
            //
